...
> Performance figures shown in this README are **illustrative only** and do not represent formally validated experimental results.
...
- **Expert semantic cues** (50-dim hand-crafted features) injected as routing priors — not concatenated with lexical features
...
Unlike flat softmax classifiers, capsule vectors preserve spatial/structural relationships within the request. The slot-controlled routing layer allows domain experts to inject prior knowledge directly into the routing process, enabling the model to generalize across dataset distributions (e.g. trained on CSIC 2010, tested on ATRDF).
...
HTTP Request
     │
     ├─ TF-IDF (1-3gram, 1500 features, L2-norm)
     │        └─> MLP (512→256, ReLU, LayerNorm, Dropout)
     │                └─> 32 Primary Capsules (8-dim)
     │                         └─> SlotControlledCapsuleLayer ──┐
     │                                                           │
     └─ Expert Cues (50-dim)                                     │
              z-scored on train                                   │
              padded to 100-slot layout                          │
              └─> injected as routing priors ────────────────────┘
                                                                  │
                                                     K Class Capsules (16-dim)
                                                                  │
...
└── src/
    ...
    ├── features/
    │   └── expert_features.py     # 50-dim expert semantic cues (Table III)
    └── evaluation/
...
from src.models.hotpatch_caps import (
    ...
    TFIDF_MAX_FEATURES, TFIDF_NGRAM_RANGE, NUM_CUE_FEATURES,
)
...
print(result)
# {'prediction': 'Log4Shell', 'confidence': 0.82, 'fired_patches': ['Log4Shell_v1'], ...}
```

---

## Expert Semantic Cues

The 50-dimensional cue vector (defined in `src/features/expert_features.py`) captures structural signals for eight attack families:

| Group          | Indices | Count | Example signals                              |
|----------------|---------|-------|----------------------------------------------|
| SQLi           | 0–9     | 10    | keyword count, tautology patterns, comments  |
| XSS            | 10–20   | 11    | `<script>`, `eval()`, event handlers         |
| Directory Traversal | 21–25 | 5  | `../`, URL-encoded traversal, sensitive paths |
| Log4Shell/JNDI | 26–33   | 8    | `${jndi:}`, remote schemes, nested interp.   |
| Log Forging    | 34–38   | 5    | CRLF injection, env variable references      |
| JNDI Placement | 39–44   | 6    | JNDI location (URL / body / header)          |
| Browser/Ctx    | 45–48   | 4    | User-Agent presence, Sec-Fetch headers        |
| Request Shape  | 49      | 1    | presence of query string                     |

These features are **injected into routing logits only** — they do not modify the TF-IDF input stream. This preserves modularity and allows the two modalities to be updated independently.

---

## Evaluation

| Setting                        | Accuracy | Macro F1 | AUPRC |
|-------------------------------|----------|----------|-------|
| In-distribution (D1)          | 0.973    | 0.961    | 0.982 |
| Cross-dataset (CSIC → D1)     | 0.914    | 0.887    | 0.931 |
| + Hot patch (Log4Shell slot)   | 0.951    | 0.929    | 0.958 |

*Results are illustrative; reproduce using your own data splits.*
